...
```{r facet box plot alt, message=FALSE, warning=FALSE, fig.width=8, fig.height=6, fig.align='center'}
#make facet
boxp <- ggplot(TextMessages_long, 
             aes(x = Timepoint, 
                 y = Value, 
                 fill = Timepoint)) +
  geom_boxplot() +
  facet_grid(. ~ Group_factor, scales = "free") +
  labs(x = "Group Factor", y = "Value", 
       title = "Comparison of Baseline and Six Months Values")
...
```{r facet bar plot alt, message=FALSE, warning=FALSE, fig.width=8, fig.height=6, fig.align='center'}
#ideal solution just need to add error bars
barp <- ggplot(TextMessages_long, 
               aes(x = Timepoint, 
                   y = Value, 
...
Looking at the error bar charts, the overlap between both 
...
much of the variability in decreased text messaging for group 1 may be attributed  
